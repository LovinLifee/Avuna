...
public final class BitBuffer {
    ...
    static {
        ...
        for (int i = 0; i < MASKS.length; i++) {
            MASKS[i] = TWO.pow(i).subtract(BigInteger.ONE).longValue();
        }
    }
    ...
    public char getChar(ByteOrder order) {
        char value = (char) getBits(Integer.SIZE);
    ...
    public int getInt(ByteOrder order) {
        int value = (int) getBits(Integer.SIZE);
    ...
    public short getShort(ByteOrder order) {
        short value = (short) getBits(Short.SIZE);
